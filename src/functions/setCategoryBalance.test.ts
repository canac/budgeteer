...
describe("setCategoryBalance", () => {
  ...
  beforeEach(async () => {
    const [budget, category]: [Budget, Category] = await Promise.all([
      createBudget({ month }),
      createCategory({ createdMonth: month, accumulating: true }),
    ]);
    categoryId = category.id;
    await Promise.all([
      prisma.budgetCategory.create({
        data: { budgetedAmount: 500, categoryId, budgetId: budget.id },
      }),
      createTransaction({
        amount: -100,
        date: "2025-01-10",
  ...
  it("creates a new balance adjustment when none exists", async () => {
    await setCategoryBalance({ data: { categoryId, month, targetBalance: 400 } });

    const adjustment = await prisma.transaction.findFirst({
      where: { type: "BALANCE_ADJUSTMENT" },
    });
    expect(adjustment).toMatchObject({ amount: 200, date: "2025-01-01" });
  });
  ...
  it("updates existing balance adjustment", async () => {
    ...
    expect(pluck(updated.transactionCategories, "amount")).toEqual([200]);
  });
  ...
  it("deletes existing adjustment when new total is zero", async () => {
    ...
    await setCategoryBalance({ data: { categoryId, month, targetBalance: 200 } });
